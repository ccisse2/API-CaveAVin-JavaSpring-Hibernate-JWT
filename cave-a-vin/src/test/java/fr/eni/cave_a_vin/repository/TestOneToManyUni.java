...
public class TestOneToManyUni {
    ...
    public void test_save_nouvelleLigne_nouveauPanier() {
        int qte = 4;
        final LignePanier lp = LignePanier.builder().quantite(qte).prix(qte * 23.95f).build();
        final Panier panier = new Panier();

        panier.setPrixTotal(lp.getPrix()); // Association OneToMany
    ...
    public void test_delete() {
        final Panier panierDB = panierEnDB();
        final List<Integer> listeIdLignePanier = panierDB.getLignesPanier().stream().map(LignePanier::getId).collect(Collectors.toList());
        // Appel du comportement
    ...
    private Panier panierEnDB() {
        final Panier panier = new Panier();
        final LignePanier lp = LignePanier.builder().quantite(3).prix(3 * 11.45f).build();
        panier.getLignesPanier().add(lp);
